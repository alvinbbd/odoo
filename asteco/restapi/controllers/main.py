...
class RestApi(http.Controller):
    ...
    def start_service(self, **kwargs):
        ...
        auth = auth_obj.search([('user_id', '=', SUPERUSER_ID)])
        if auth:
            ...
            auth = auth_obj.create({'name': 'Admin Auth', 'user_id': SUPERUSER_ID})
            auth_data = {'client_id': auth.consumer_key,
                'client_secret': auth.consumer_secret,
                'host': request.httprequest.url_root,
                'database': request.cr.dbname}
            return self.get_response(200, '200', auth_data)
