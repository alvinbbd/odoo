...
select to_timestamp(lt.available_from, 'DD-MM-YYYY hh24:mi'),reference,unit,
...
NULL,NULL,NULL,
NULL,1,to_timestamp(lt.last_updated_date_and_time, 'DD-MM-YYYY hh24:mi'),1,TRUE,
to_timestamp(lt.created_date_and_time, 'DD-MM-YYYY hh24:mi')
from listing_temp lt limit 5;
...
FROM   listing_temp  iwvs
JOIN   res_partner iv ON trim(lower(iv.name)) = trim(lower(iwvs.contact))
WHERE  iw.ref_name = iwvs.reference;
...
from res_partner rp
join res_users ru
on ru.partner_id=rp.id) iv ON trim(lower(iv.name)) = trim(lower(iwvs.created_by))
WHERE  iw.ref_name = iwvs.reference;



*******************************************LISTING WITHOUT MAINTENANCE FEE***********************************
update listing_temp set listing_type='Sale' where listing_type='Sales';
update listing_temp set featured='true' where featured='Yes';
update listing_temp set featured='false' where featured='No';
update listing_temp set property_tenanted='true' where property_tenanted='Yes';
...
insert into res_location(name,emirate_id,create_uid,create_date,write_uid,write_date)
select distinct lt.listing_location,1,1,CURRENT_TIMESTAMP,1,CURRENT_TIMESTAMP from listing_temp lt
where trim(lower(lt.listing_location)) not in (select trim(lower(name)) from res_location);
/*update emirates in location table*/
UPDATE res_location iw
SET    emirate_id = iv.id
FROM   listing_temp  iwvs
...
CREATE TABLE public.listing_temp
(
  status text,
  reference text,
  listing_type text,
...
update listing_temp set price_per_sqft = NULL where price_per_sqft = '- -';
...
NULL,NULL,NULL,NULL,NULL,NULL,NULL,
NULL,NULL,NULL,
NULL,3,to_timestamp(lt.last_updated_date_and_time, 'MM-DD-YYYY hh24:mi'),1,TRUE,
